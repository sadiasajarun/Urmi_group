...
### Modal Component (MANDATORY: react-hook-form)

> **Modal and drawer forms MUST use react-hook-form + Zod. Call `form.reset()` when the modal closes. For edit forms, use `useEffect` to call `form.reset(existingData)` when the entity changes.**

```tsx
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { Form, FormField, FormItem, FormLabel, FormControl, FormMessage } from '~/components/ui/form';
import { createSchema, type CreateFormData } from '~/utils/validations/feature';
