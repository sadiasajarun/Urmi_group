...
const { resolve, dirname, relative, basename, join } = path;
...
try {
  ...
  // TypeScript not available - will skip import analysis
}
...
function isFileRecent(filePath, thresholdHours) {
  try {
...
function isFileExcluded(filePath, config, projectDir) {
...
function buildImportGraph(files, projectDir) {
  ...
  // Skip import analysis if TypeScript not available
  if (!ts) {
...
function findUnusedImports(files, graph, config, projectDir) {
  const unused = [];
...
function findOrphanedTests(files, config, projectDir) {
  const orphaned = [];
...
function findUnusedDTOs(files, graph, config, projectDir) {
  const unused = [];
...
function deleteFiles(filePaths, config) {
  const result = {
    deleted: [],
    errors: [],
  };
...
function removeEmptyDirectories(rootPath, config) {
  const removed = [];
...
function main() {
  try {
    ...
    if (!ts) {
      console.error('[Warning: TypeScript not found - import analysis disabled]');
      console.error('');
    }
    ...
    const unusedFiles = ts ? findUnusedImports(allFiles, graph, config, projectDir) : [];
    const orphanedTests = findOrphanedTests(allFiles, config, projectDir);
    const unusedDTOs = ts ? findUnusedDTOs(allFiles, graph, config, projectDir) : [];
    ...
    if (process.env.DEBUG) {
      console.error(
        `[cleanup] Error: ${error.message || 'Unknown error'}`
      );
      if (error.stack) {
